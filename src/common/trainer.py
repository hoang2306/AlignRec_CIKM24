...
class Trainer(AbstractTrainer):
   ...
   and some other features helpful for model training and evaluation.
    ...
    def __init__(self, config, model):
        ...
        self.warmup_ui = config['warmup_ui']
        self.warmup_epoch = config['warmup_epoch']
        if self.warmup_ui:
            # warm up with just train ui-modality loss
            self.warmup_epoch = 10
            def fac_func(epoch=0):
                if epoch<self.warmup_epoch:
                    return (epoch+1)/self.warmup_epoch
                else:
                    return lr_scheduler[0] ** (epoch / lr_scheduler[1])
            scheduler = optim.lr_scheduler.LambdaLR(self.optimizer, lr_lambda=fac_func)
        else:
            scheduler = optim.lr_scheduler.LambdaLR(self.optimizer, lr_lambda=fac)
        self.save_model = config['save_model']
        self.log_file_name = os.path.join(config['model'],config['dataset'],config['log_file_name'])
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None):
        ...
        for batch_idx, interaction in enumerate(train_data):
            ...
            # warm up by only train u-i loss
            if self.warmup_ui and epoch_idx<self.warmup_epoch:
                losses = loss_func(interaction, not_train_ui=True)
            else:
    ...
    def fit(self, train_data, valid_data=None, test_data=None, saved=False, verbose=True):
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            for param_group in self.optimizer.param_groups:
               self.logger.info('======lr: {}'.format(param_group['lr']))
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                if update_flag:
                    ...
                    # if self.save_model:
                    #     # we only support save one model for each hyper-para combination
                    #     saved_dir = os.path.join('saved_models',self.log_file_name)
                    #     if not os.path.exists(saved_dir):
                    #         os.makedirs(saved_dir)
                    #     with torch.no_grad():
                    #         res = self.model.forward(self.model.norm_adj,train=True)
                    #     np.save(os.path.join(saved_dir,'mm_emb.npy'),res[2].detach().cpu().numpy())
                    #     np.save(os.path.join(saved_dir,'id_emb.npy'),res[3].detach().cpu().numpy())
